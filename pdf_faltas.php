...
//if($turma != 'Nenhum'){
	$sql = "select Codigo, Nome,Turma, COUNT(*) from entrada WHERE Turma='$turma' and Escola = '$escola'AND Dia between('$data') and ('$data_ate') GROUP BY Nome ORDER BY count(*) DESC LIMIT 30";
//}else{
	//$sql = "select Codigo, Nome,Turma, count(*)  from entrada WHERE Dia between('$data') and ('$data_ate') GROUP BY Nome ORDER BY count(*) DESC ORDER BY Turma LIMIT 30";
	//$sql = "select * from entrada where Entrada = 'NAO' AND Dia between('2019-01-16') and ('2019-01-30') ORDER BY Turma LIMIT 200";
//}
...
$html .= '
	...
	$html .= '<th style="text-align:center">Faltas</th>';
	$html .= '</tr>';
	$html .= '</thead>';
	$html .= '<tbody>';
	...
	while($rows_livros = mysqli_fetch_assoc($resultado_livros)){
		...
		$html .= '<td style="">'.$rows_livros['Nome'] . "</td>";
	...
	<h6 style="text-align:center">RELATÓRIO DE FALTAS</h6>
  ...
  '.$html.'
	...
	$dompdf->render();
	//Exibibir a página
	$dompdf->stream("faltas.pdf",array("Attachment" => false));
?>