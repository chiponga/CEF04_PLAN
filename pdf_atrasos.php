...
$html .= '
	...
	$html .= '<th style="text-align:center">Atrasos</th>';
	$html .= '</tr>';
	$html .= '</thead>';
	$html .= '<tbody>';
		...
		$escola = $objDb->nome;
		$sql = "SELECT Codigo, Nome,Turma, count(*) FROM entrada WHERE Entrada = 'ATRASADO(A)' AND Turma='$turma' and Escola = '$escola' AND Dia between('$data') and ('$data_ate') GROUP BY Nome ORDER BY count(*) DESC";
	...
	<h6 style="text-align:center">RELATÓRIO DE ATRASOS</h6>
  ...
  '.$html.'
	...
	//Exibibir a página
	$dompdf->stream("atrasos.pdf",array("Attachment" => false));
